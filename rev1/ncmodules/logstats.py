...
def get_health_html(dash_data):
    ...
    pressurestr = f"<div class='health-div'>\n <h3 title='{t}'>PRESSURE</h3>\n <p class={get_class(dash_data,"int_Pressure")}>{str(f"{round(dash_data["int_Pressure"], 2):.2f}")} psi</p>\n </div>\n"
    ...
    batterystr = f"<div class='health-div'>\n <h3 title='{t}'>BATTERY</h3>\n <p class={get_class(dash_data,"batteryPercent")}>{str(f"{round(dash_data["batteryPercent"]*100, 1):.1f}")} % \
       ({int(dash_data["battery_ndives"] * batsf)} dives | {str(f"{((dash_data["battery_ndives"] * batsf * dash_data["glider_dive_time"]/3600) / 24.0):.2f}")} days )</p>\n </div>\n"
...
def get_navigation_html(dash_data):
    ...
    positionstr = f"<div class='nav-div'>\n <h3>POSITION</h3>\n <p class='p-normal'>{str(f"{round(dash_data["gps_lat_end"], 4):.4f}")}, \
                    {str(f"{round(dash_data["gps_lon_end"], 4):.4f}")}</p>\n </div>\n" 
       
    targetstr = f"<div class='nav-div'>\n <h3>TARGET</h3>\n <p class='p-normal'> <p style=\"font-weight: bold; font-style: italic;\">{dash_data["TGT_name"]}</p>{str(f"{round(dash_data["TGT_lat"], 4):.4f}")}, \
                    {str(f"{round(dash_data["TGT_lon"], 4):.4f}")} <br> DTT: {str(f"{round(dash_data["tgt_distance"], 2):.2f}")} km \
                    <br>TTG: {str(f"{round(dash_data["tgt_time"], 1):.1f}")} h ({str(f"{round(dash_data["tgt_time"]/24, 1):.1f}")} d)</p>\n </div>\n"
    ...
    dogstr = f"<div class='nav-div'>\n <h3>DOG</h3>\n <p class={get_class(dash_data,"glider_dog")}>{str(round(dash_data["glider_dog"],2))} km | \
                    ...
                    Off to target by: {str(f"{round(dash_data["tgt_direction"] - dash_data["glider_hdg"],0):.0f}")} {chr(176)}</div>\n"

    divetime = f"<div class='nav-div'>\n <h3>DIVE TIME</h3>\n <p class={get_class(dash_data,"glider_dive_time")}> \
        {str(round(dash_data["glider_dive_time"]/60.0,0))} min ({str(round(dash_data["glider_dive_time"]/3600.0,1))} h) <br><br> \
...
def get_rates_html(dash_data):
    ...
    vbdrateminstr = f"<div class='rates-div'>\n <h3>VBD RATE MIN</h3>\n <p class={get_class(dash_data,"vbd_rate_min")}>{str(round(dash_data["vbd_rate_min"],2))} AD/s</p>\n </div>\n"
    vbdrateapstr = f"<div class='rates-div'>\n <h3>VBD RATE APOGEE</h3>\n <p class={get_class(dash_data,"vbd_rate_apogee")}>{str(round(dash_data["vbd_rate_apogee"],2))} AD/s</p>\n </div>\n"
    # roll
