...
def elapsed(dt):
    ...
    dive_time = datetime(int(y), int(m), int(d), int(h), int(mm), int(s))
    current_time = datetime.now(timezone.utc) 
    elapsed_time = (current_time.timestamp() - dive_time.timestamp())/3600.0 # in h
...
# extracts last 10 dives of each dataframe
...
def make_summary_map(df_list):
    ...
    full_map.add_child(MeasureControl())
