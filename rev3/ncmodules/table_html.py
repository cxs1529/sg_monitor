...
def convert_to_table(df):
    ...
    filter_headers = ['dive', 'time_start', 'time_end', 'gps_lat_start', 'gps_lon_start',
       ...
       'vbd_rate_min', 'vbd_i_apogee', 'vbd_rate_apogee', 'depth_reached', 
        "log_AH_total_capacity", "log_AH_total_consumed", "log_24_minv", "log_10_minv", "log_AH_thisdive" ] # 'filePath' removed
    ...
    for j in df.keys():
        if j in filter_headers:
            ...
            if(j == "ERRORS"):
                tip = "pitch_error,roll_error,vbd_error,pitch_retry,roll_retry,vbd_retry,gps_timeout,compass_timeout,sensor_timeout[0-1-2-3]"
                html_table_headers = html_table_headers + f"\n\t<th class=\"table_header\" title='{tip}'> {j} {units} </th>"
            else:
...
def get_data_class(i, col, df):
    ...
    gps_locations = ['gps_lat_start', 'gps_lon_start', 'gps_lat_end', 'gps_lon_end', 'TGT_lat', 'TGT_lon']
    ...
    battery_ah = ["log_AH_total_capacity", "log_AH_total_consumed", "log_AH_thisdive"]
    ...
    if col == "int_Humidity":
        ...
        if abs(data_value) <= SM_angle_thd["min"] or abs(data_value) >= SM_angle_thd["max"]:
            data_class = "data_warning"        
    elif col == "SM_depth":
        data_value = round(data_value, 1)
        ...
        if data_value <= vbd_rate_thd["min"] or data_value >= vbd_rate_thd["max"]:
            data_class = "data_warning" 
        data_value = f"{round(data_value, 2):.2f}"
