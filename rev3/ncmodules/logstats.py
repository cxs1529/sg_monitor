...
def glider_stats(df):
    ...
    log_AH_thisdive = df.loc[0,"log_AH_thisdive"]
    battery_ndives = (log_AH_total_capacity - log_AH_total_consumed) / log_AH_thisdive # number of dives to do at current energy consumption (based on last dive)
    ...
    # Extract raw error elements split by comma
    raw_errors = str(df.loc[0, "ERRORS"]).split(",")

    # Clean array: handles both "key=val" (Format 1 in SGX) and raw "val" (Format 2 in SG)
    errors = [item.split("=")[-1].strip() for item in raw_errors]

    # Map parsed values to the dictionary
    error_dict = {
        "pitch_error": errors[0] if len(errors) > 0 else "0",
        "roll_error": errors[1] if len(errors) > 1 else "0",
        "vbd_error": errors[2] if len(errors) > 2 else "0",
        "pitch_retry": errors[3] if len(errors) > 3 else "0",
        "roll_retry": errors[4] if len(errors) > 4 else "0",
        "vbd_retry": errors[5] if len(errors) > 5 else "0",
        "gps_timeout": errors[6] if len(errors) > 6 else "0",
        "compass_timeout": errors[7] if len(errors) > 7 else "0",
        "sensor_timeout": (
            "-".join(errors[8:12]) if len(errors) >= 12 else "0-0-0-0"
        ),
    }
